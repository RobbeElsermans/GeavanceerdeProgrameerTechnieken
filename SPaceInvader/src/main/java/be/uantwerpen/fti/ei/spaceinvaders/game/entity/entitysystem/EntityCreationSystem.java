...
public class EntityCreationSystem {
    ...
    public boolean bonusCreation(List<ABonusEntity> be, List<StatisticsComponent> sc, AFactory gfx, IDimension gameDimention) {
        ...
        if (be.stream().noneMatch(i -> i.getCollectableComponent().getType() == CollectableType.MOVE_SPEED)) {
            //Heeft een speler shotHit van %20 bereikt? maak dan een bonus van movespeed van 0.5 dus versnellen.
            if (!hasFiredBonus0 && sc.stream().anyMatch(i -> (i.getShotsHits() % 2 == 0) && (i.getShotsHits() > 0))) {
                be.add(gfx.getBonusEntity(new Position(Random.getRandom((int) gameDimention.getWidth()), 0), 2, 0.5, CollectableType.MOVE_SPEED, 0.5));
                hasFiredBonus0 = true;
                return true;
            } else if (sc.stream().noneMatch(i -> (i.getShotsHits() % 2 == 0) && (i.getShotsHits() > 0))) {
                hasFiredBonus0 = false;
            }

            //Heeft een speler shotMiss van %20 bereikt? maak dan een bonus van movespeed van -0.5 dus vertragen.
            if (!hasFiredBonus1 && sc.stream().anyMatch(i -> (i.getShotsMissed() % 20 == 0) && (i.getShotsMissed() > 0))) {
                be.add(gfx.getBonusEntity(new Position(Random.getRandom((int) gameDimention.getWidth()), 0), 4, 0.5, CollectableType.MOVE_SPEED, -0.5));
                hasFiredBonus1 = true;
                return true;
            }
